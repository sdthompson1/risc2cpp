...
-- Minimal prologue for additional .cpp files when splitting
cppMinimalPrologue :: String -> [String]
cppMinimalPrologue hppFilename =
    ["// This file was generated by Risc2cpp"
    ,""
    ,"#include \"" ++ hppFilename ++ "\""
    ,""
...
cppPrologue hppFilename programBreak startAddr startBlock =
    ...
    ,"        if (isPageAllocated(getGuardPageAddress() - BYTES_PER_PAGE)) {"
...
initCaseTable addrToCase addrs = ["// CASE TABLE"
                                 ...
                                 ,"    };"
                                 ,""
                                 ,"    const CaseTableItem CASE_TABLE[] = {"]
                                 ++ map (\a -> "        {" ++ toHex a ++ ", " ++ show (addrToCase Map.! a) ++ "},") addrs
                                 ++ ["   };"
                                    ,"}"
                                    ,""
                                    ,"void RiscVM::initCaseTable()"
                                    ,"{"
                                    ,"    for (const CaseTableItem &item : CASE_TABLE) {"
                                    ,"        case_table[item.addr] = item.case_label;"
                                    ,"    }"
                                    ,"}"
                                    ,""]

codeGen :: String -> Maybe Int -> Map Addr [Statement] -> [Addr] -> [(Word32, [Word32])] -> Word32 -> Word32 -> ([String], [[String]])
codeGen hppFilename splitCpp blocks indirectJumpTargets dataChunks entryPoint programBreak =
    let blockMap = makeBlockNumsMap blocks
        codeChunks = groupsOf numBlocksPerMethod (Map.toList blocks)
        allMethods = map (\(x,n) -> makeExecMethod n x blockMap) (zip codeChunks [0, numBlocksPerMethod ..])

        hpp = hppContents hppFilename (length codeChunks)
        cpp0 = cppPrologue hppFilename programBreak entryPoint (blockMap Map.! entryPoint)
                ++ makeTrampoline (length codeChunks)
                ++ (case splitCpp of Nothing -> concat allMethods; Just _ -> [])
                ++ initCaseTable blockMap (uniq $ sort indirectJumpTargets)
                ++ initMem dataChunks
        cppOthers = case splitCpp of
                      Nothing -> []
                      Just n -> map (\methodGroup -> cppMinimalPrologue hppFilename ++ concat methodGroup) (groupsOf n allMethods)
